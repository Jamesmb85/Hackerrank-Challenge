...
int* rotateLeft(int d, int arr_count, int* arr, int* result_count) {
    //assign the size of the array to result_count
    *result_count = arr_count;
    ...
    for(int i = 0; i < d; i++){
        ...
        //need the value at the beginning of the array
        int current_first_element = arr[0];
        for(int j = 0; j < arr_count - 1; j++){
            arr[j] = arr[j+1];
        }
        //swap the first element
        arr[arr_count - 1] = current_first_element;
    }
